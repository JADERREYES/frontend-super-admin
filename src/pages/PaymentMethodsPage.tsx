...
export function PaymentMethodsPage() {
  ...
  const load = async () => {
    try {
      ...
      setError(err?.response?.data?.message || 'No se pudieron cargar los metodos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const save = async () => {
    try {
      ...
      setFormError(err?.response?.data?.message || 'No se pudo guardar el metodo');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Modal open={editorOpen} title={selected ? 'Editar metodo de pago' : 'Nuevo metodo de pago'} onClose={closeEditor}>
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Proveedor</span><input value={draft.provider} onChange={(e) => setDraft((prev: any) => ({ ...prev, provider: e.target.value }))} className={inputClassName} /></label>
            <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Tipo</span><input value={draft.type} onChange={(e) => setDraft((prev: any) => ({ ...prev, type: e.target.value }))} className={inputClassName} /></label>
            <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Etiqueta cuenta</span><input value={draft.accountLabel} onChange={(e) => setDraft((prev: any) => ({ ...prev, accountLabel: e.target.value }))} className={inputClassName} /></label>
            <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Valor cuenta</span><input value={draft.accountValue} onChange={(e) => setDraft((prev: any) => ({ ...prev, accountValue: e.target.value, accountNumber: e.target.value }))} className={inputClassName} /></label>
            <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Titular</span><input value={draft.holderName || draft.accountHolder} onChange={(e) => setDraft((prev: any) => ({ ...prev, holderName: e.target.value, accountHolder: e.target.value }))} className={inputClassName} /></label>
          ...
          <label className="block space-y-2"><span className="text-sm font-medium text-slate-700">Instrucciones</span><textarea rows={5} value={draft.instructions} onChange={(e) => setDraft((prev: any) => ({ ...prev, instructions: e.target.value }))} className={inputClassName} /></label>
